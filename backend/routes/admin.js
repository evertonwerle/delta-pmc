...
function can(req){ return adminAuth(req); }
function actor(req){
...
function log(req, acao, entidade, entidadeId, detalhes=''){
  const a=actor(req);
  db.run(`INSERT INTO logs_sistema (usuario_tipo,usuario_id,usuario_nome,acao,entidade,entidade_id,detalhes) VALUES (?,?,?,?,?,?,?)`,[a.tipo,a.id,a.nome,acao,entidade,entidadeId||null,detalhes]);
}
router.get('/overview',(req,res)=>{
  if(!can(req)) return res.status(403).json({erro:'Acesso administrativo negado.'});
  try{
    const q=s=>Number(db.get(s)?.total||0);
    const cargos=db.all(`SELECT cargo_delta cargo,COUNT(*) total FROM users WHERE ativo=1 GROUP BY cargo_delta ORDER BY CASE cargo_delta WHEN 'GESTOR' THEN 1 WHEN 'SUB-GESTOR' THEN 2 WHEN 'COORDENADOR' THEN 3 WHEN 'PILOTO MASTER' THEN 4 WHEN 'PILOTO DE ELITE' THEN 5 WHEN 'PILOTO ESPECIALISTA' THEN 6 WHEN 'PILOTO AVANÇADO' THEN 7 WHEN 'PILOTO ASPIRANTE' THEN 8 WHEN 'PILOTO PROBATORIO' THEN 9 ELSE 10 END`);
    const stats={
      usuarios:q(`SELECT COUNT(*) total FROM users`),
      candidatos:q(`SELECT COUNT(*) total FROM candidaturas WHERE status IN ('PENDENTE','APROVADO','REPROVADO')`),
      emAnalise:q(`SELECT COUNT(*) total FROM candidaturas WHERE status='PENDENTE'`),
      aprovados:q(`SELECT COUNT(*) total FROM candidaturas WHERE status='APROVADO'`),
      reprovados:q(`SELECT COUNT(*) total FROM candidaturas WHERE status='REPROVADO'`),
      pilotosAtivos:q(`SELECT COUNT(*) total FROM users WHERE ativo=1 AND cargo_delta NOT IN ('GESTOR','SUB-GESTOR','COORDENADOR')`),
      afastados:q(`SELECT COUNT(*) total FROM membros WHERE status='AFASTADO'`),
      exonerados:q(`SELECT COUNT(*) total FROM exoneracoes`),
      apreensoes:q(`SELECT COUNT(*) total FROM apreensoes`),
      horasSegundos:Number(db.get(`SELECT COALESCE(SUM(CASE WHEN saida IS NULL THEN (julianday('now')-julianday(entrada))*86400 ELSE (julianday(saida)-julianday(entrada))*86400 END),0) total FROM pontos`)?.total||0),
      acoesFechadas:q(`SELECT COUNT(*) total FROM relatorios_acoes`)
    };
    const recentes=db.all(`SELECT id,usuario_nome,acao,entidade,entidade_id,detalhes,criado_em FROM logs_sistema ORDER BY id DESC LIMIT 12`);
    const usuarios=db.all(`SELECT id,nome,username,cargo_delta,ativo,status_conta,criado_em,ultimo_login FROM users ORDER BY id DESC LIMIT 8`);
    const candidaturas=db.all(`SELECT c.id,c.personagem,c.id_jogador,c.status,c.etapa_liberada,c.criado_em,u.nome FROM candidaturas c LEFT JOIN users u ON u.id=c.usuario_id ORDER BY c.id DESC LIMIT 8`);
    const exon=db.all(`SELECT id,nome,cargo_no_momento,nivel,motivo,ocorrido_em FROM exoneracoes ORDER BY id DESC LIMIT 8`);
...
router.get('/usuarios',(req,res)=>{
  ...
  try{
    ...
    res.json({usuarios:db.all(sql,p)});
...
router.get('/usuarios/:id',(req,res)=>{
  if(!can(req)) return res.status(403).json({erro:'Acesso administrativo negado.'});
  try{
    const id=Number(req.params.id); const u=db.get(`SELECT id,username,nome,cargo_delta,ativo,status_conta,criado_em,ultimo_login,inscricao_enviada FROM users WHERE id=?`,[id]);
    if(!u) return res.status(404).json({erro:'Usuário não encontrado.'});
    const candidaturas=db.all(`SELECT * FROM candidaturas_historico WHERE usuario_id=? ORDER BY id DESC`,[id]);
    const atuais=db.all(`SELECT id,personagem,id_jogador,patente,etapa,etapa_liberada,status,observacao,criado_em,atualizado_em FROM candidaturas WHERE usuario_id=? ORDER BY id DESC`,[id]);
    const cargos=db.all(`SELECT * FROM historico_cargos WHERE usuario_id=? ORDER BY id DESC`,[id]);
    const exoneracoes=db.all(`SELECT * FROM exoneracoes WHERE usuario_id=? ORDER BY id DESC`,[id]);
    const apreensoes=db.all(`SELECT * FROM apreensoes WHERE usuario_id=? ORDER BY id DESC`,[id]);
    const pontos=db.all(`SELECT id,entrada,saida,ROUND(CASE WHEN saida IS NULL THEN (julianday('now')-julianday(entrada))*86400 ELSE (julianday(saida)-julianday(entrada))*86400 END,0) segundos FROM pontos WHERE usuario_id=? ORDER BY id DESC`,[id]);
    const logs=db.all(`SELECT * FROM logs_sistema WHERE usuario_id=? ORDER BY id DESC LIMIT 100`,[id]);
...
function salvarCargoAdmin(req, res) {
  if (!can(req)) return res.status(403).json({ sucesso:false, erro:'Acesso administrativo negado.' });
  ...
  try {
    const antes = db.get('SELECT id,username,nome,cargo_delta,ativo,status_conta FROM users WHERE id=? LIMIT 1',[id]);
    ...
    const depois = db.get('SELECT id,username,nome,cargo_delta,ativo,status_conta FROM users WHERE id=? LIMIT 1',[id]);
    ...
    try {
      if (String(antes.cargo_delta || '').trim().toUpperCase() !== cargo) {
        db.run(`INSERT INTO historico_cargos
      ...
      log(req,'ALTERAÇÃO DE CARGO','USUARIO',id,`${antes.cargo_delta || 'SEM CARGO'} → ${cargo}`);
    } catch (auditError) {
...
router.get('/usuarios/:id/cargo',(req,res)=>{
  if (!can(req)) return res.status(403).json({sucesso:false,erro:'Acesso administrativo negado.'});
  const id=Number(req.params.id);
  const usuario=db.get('SELECT id,username,nome,cargo_delta,ativo,status_conta FROM users WHERE id=? LIMIT 1',[id]);
...
router.patch('/usuarios/:id',(req,res)=>{
  if(!can(req)) return res.status(403).json({erro:'Acesso administrativo negado.'});
  try{
    const id=Number(req.params.id), u=db.get(`SELECT * FROM users WHERE id=?`,[id]); if(!u)return res.status(404).json({erro:'Usuário não encontrado.'});
    ...
    db.run(`UPDATE users SET nome=?,ativo=? WHERE id=?`,[nome,ativo,id]);
    log(req,'ALTERAÇÃO DE USUÁRIO','USUARIO',id,`Nome/status atualizados: ${nome} / ${ativo?'ATIVO':'INATIVO'}`);
...
router.get('/ponto',(req,res)=>{
  ...
  try{
    ...
    const registros=db.all(sql,ps); const total=registros.reduce((s,r)=>s+Number(r.segundos||0),0); res.json({registros,total_segundos:Math.round(total)});
