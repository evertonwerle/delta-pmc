...
router.post('/register', (req, res) => {
  try {
    ...
    const existeAdmin = db.get('SELECT id FROM admins WHERE username = ? LIMIT 1', [usuario]);
    const existeUser = db.get('SELECT id FROM users WHERE username = ? LIMIT 1', [usuario]);
...
router.post('/login', (req, res) => {
  try {
    ...
    const admin = db.get('SELECT * FROM admins WHERE username = ? AND ativo = 1 LIMIT 1', [usuario]);
    if (admin && bcrypt.compareSync(senha, admin.password_hash)) {
      ...
      db.run('UPDATE admins SET ultimo_login = datetime(\'now\') WHERE id = ?', [admin.id]);
      db.run(`INSERT INTO logs_sistema (usuario_tipo,usuario_id,usuario_nome,acao,entidade,entidade_id,detalhes) VALUES (?,?,?,?,?,?,?)`,['ADMINISTRADOR',admin.id,admin.nome||admin.username,'LOGIN','ADMIN',admin.id,'Login administrativo realizado']);
      return res.json({ sucesso: true, role: 'admin', admin: req.session.admin });
    }

    const user = db.get("SELECT * FROM users WHERE username = ? AND (ativo = 1 OR status_conta = 'EXONERADO') LIMIT 1", [usuario]);
    if (user && bcrypt.compareSync(senha, user.password_hash)) {
      ...
      const aprovado = db.get(`SELECT id FROM candidaturas WHERE usuario_id = ? AND status = 'APROVADO' LIMIT 1`, [user.id]);
      ...
      db.run(`INSERT INTO logs_sistema (usuario_tipo,usuario_id,usuario_nome,acao,entidade,entidade_id,detalhes) VALUES (?,?,?,?,?,?,?)`,[String(user.cargo_delta||'PILOTO').toUpperCase(),user.id,user.nome,'LOGIN','USUARIO',user.id,'Login realizado']);
...
router.post('/logout', (req, res) => {
  ...
  try{db.run(`INSERT INTO logs_sistema (usuario_tipo,usuario_id,usuario_nome,acao,entidade,entidade_id,detalhes) VALUES (?,?,?,?,?,?,?)`,[a[0],a[1]||null,a[2]||'', 'LOGOUT','SESSAO',a[1]||null,'Logout realizado']);}catch(e){}
...
router.get('/me', (req, res) => {
  ...
  if (req.session.user) {
    const row = db.get('SELECT id, username, nome, cargo_delta, ativo, inscricao_enviada, status_conta FROM users WHERE id = ? LIMIT 1', [req.session.user.id]);
    if (!row || (Number(row.ativo) !== 1 && String(row.status_conta || '').toUpperCase() !== 'EXONERADO')) {
      return res.json({ autenticado: false, role: null });
    }
    const aprovado = db.get(`SELECT id FROM candidaturas WHERE usuario_id = ? AND status = 'APROVADO' LIMIT 1`, [row.id]);
