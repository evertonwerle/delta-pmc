...
router.get('/', (req, res) => {
  try {
    ...
    if (privileged) {
      const rows = db.all(`SELECT id, categoria, titulo, conteudo, ordem, ativo, atualizado_em FROM documentos WHERE ativo = 1 ORDER BY categoria, ordem, id`);
      return res.json({ documentos: rows, acesso_manual: true, acesso_apostila: true });
    }

    const userId = req.session?.user?.id;
    if (!userId) {
      const rows = db.all(`SELECT id, categoria, titulo, conteudo, ordem, ativo, atualizado_em FROM documentos WHERE ativo = 1 AND categoria = 'EDITAL' ORDER BY ordem, id`);
      return res.json({ documentos: rows, acesso_manual: false, acesso_apostila: false });
    }

    const candidatura = db.get(`SELECT status FROM candidaturas WHERE usuario_id = ? ORDER BY id DESC LIMIT 1`, [userId]);
    ...
    const rows = categoriaPermitida
      ? db.all(`SELECT id, categoria, titulo, conteudo, ordem, ativo, atualizado_em FROM documentos WHERE ativo = 1 AND categoria IN ('EDITAL', ?) ORDER BY categoria, ordem, id`, [categoriaPermitida])
      : db.all(`SELECT id, categoria, titulo, conteudo, ordem, ativo, atualizado_em FROM documentos WHERE ativo = 1 AND categoria = 'EDITAL' ORDER BY ordem, id`);
...
router.get('/admin', hierarchyAuth, (req, res) => {
  try {
    const rows = db.all(`SELECT id, categoria, titulo, conteudo, ordem, ativo, atualizado_em FROM documentos ORDER BY categoria, ordem, id`);
...
router.post('/', contentAuth, (req, res) => {
  try {
    ...
    const result = db.run(`INSERT INTO documentos (categoria,titulo,conteudo,ordem,ativo,atualizado_em) VALUES (?,?,?,?,1,datetime('now'))`, [categoria,titulo,conteudo,ordem]);
...
router.patch('/:id', contentAuth, (req, res) => {
  try {
    const id = Number(req.params.id);
    const atual = db.get('SELECT * FROM documentos WHERE id = ?', [id]);
    ...
    if (!titulo || !conteudo) return res.status(400).json({ erro: 'Título e conteúdo são obrigatórios.' });
    if (categoria === 'ETAPA' && ![1,2,3].includes(ordem)) return res.status(400).json({ erro: 'Etapa deve ser 1, 2 ou 3.' });
    db.run(`UPDATE documentos SET categoria=?, titulo=?, conteudo=?, ordem=?, ativo=?, atualizado_em=datetime('now') WHERE id=?`, [categoria,titulo,conteudo,ordem,ativo,id]);
    res.json({ sucesso: true });
  } catch (e) { console.error(e); res.status(400).json({ erro: e.message || 'Não foi possível atualizar.' }); }
});

router.delete('/:id', contentAuth, (req, res) => {
  try {
    const id = Number(req.params.id);
    const atual = db.get('SELECT id, categoria, titulo FROM documentos WHERE id = ?', [id]);
    if (!atual) return res.status(404).json({ erro: 'Conteúdo não encontrado.' });
    ...
    const result = db.run('DELETE FROM documentos WHERE id = ?', [id]);
